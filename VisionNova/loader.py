...
class Loader:
    ...
    def load_image(image_path):
        ...
        if image.shape[-1] != CHANNELS:
            raise RuntimeError(
                f"INTERNAL ERROR: Failed to convert image to 'RGB'. Image has {image.shape[-1]} channels."
            )
        if image.shape[0] < TILE_HEIGHT or image.shape[1] < TILE_WIDTH:
            raise ValueError(
                f"Invalid image size. Image should be at least {TILE_HEIGHT}x{TILE_WIDTH} pixels, but got {image.shape[0]}x{image.shape[1]} pixels."
            )
